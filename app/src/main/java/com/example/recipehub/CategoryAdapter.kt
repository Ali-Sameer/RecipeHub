...
import com.example.recipehub.databinding.CategoryRvBinding
import com.example.recipehub.databinding.PopularRvItemBinding

class CategoryAdapter(var dataList: ArrayList<Recipe>, var context: Context) :
    RecyclerView.Adapter<CategoryAdapter.ViewHolder>() {
    inner class ViewHolder(var binding: CategoryRvBinding) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
      ...
      var temp = dataList.get(position).ing.split("\n").dropLastWhile { it.isEmpty() }.toTypedArray()
        holder.binding.time.text = temp.get(0)
        holder.binding.nextDetails.setOnClickListener {
